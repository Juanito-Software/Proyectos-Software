//! Servidor FTP mínimo: solo std, sin crates externos.
//! Soporta USER, PASS, SYST, PWD, CWD, TYPE I, PASV, LIST, RETR, STOR, QUIT.
...
/// Escribe una línea de respuesta FTP (terminada en CRLF).
fn send_reply(stream: &mut TcpStream, code: u16, message: &str) -> io::Result<()> {
    let line = format!("{code} {message}\r\n");
    stream.write_all(line.as_bytes())
}

/// Lee una línea terminada en CRLF del socket de control.
...
/// Convierte IpAddr y puerto a formato PASV: (h1,h2,h3,h4,p1,p2).
...
/// Envía listado simple al socket de datos (estilo ls -l).
fn handle_list(data_stream: &mut TcpStream, dir: &Path) -> io::Result<()> {
    ...
    for entry in entries {
        ...
        let line = if meta.is_dir() {
            ...
            format!(
                "-rw-r--r-- 1 owner group {} Jan 1 00:00 {name}\r\n",
                meta.len()
            )
        };
...
/// Envía un archivo por el socket de datos (RETR).
...
/// Recibe un archivo desde el socket de datos (STOR).
...
/// Maneja una sesión FTP en una conexión de control.
    ...
    loop {
        ...
        match cmd.as_str() {
            "USER" => {
                ...
                send_reply(&mut stream, 331, "Usuario OK, contraseña requerida")?;
            }
            "PASS" => {
                if last_user.is_some() {
                    ...
                    send_reply(&mut stream, 503, "Primero envíe USER")?;
                }
            }
            "SYST" => {
                send_reply(&mut stream, 215, "UNIX Type: L8")?;
            }
            "PWD" => {
                let rel = current_dir
                ...
                let msg = format!("\"{}\" es el directorio actual", shown);
                send_reply(&mut stream, 257, &msg)?;
            }
            "CWD" => {
                if !logged_in {
                ...
                let rel = arg.trim_start_matches('/');
                new_path.push(rel);
                let canon = fs::canonicalize(&new_path).unwrap_or(new_path);
            ...
            "PASV" => {
                ...
                let msg = format!("Entrando en modo pasivo ({})", pasv_str);
                send_reply(&mut stream, 227, &msg)?;
            ...
            "LIST" => {
                ...
                send_reply(&mut stream, 150, "Abriendo conexión de datos para LIST")?;
            ...
            "RETR" => {
                ...
                send_reply(&mut stream, 150, "Abriendo conexión de datos para RETR")?;
                let (mut data_stream, _) = listener.accept()?;
            ...
            "STOR" => {
                ...
                let mut path = current_dir.clone();
                path.push(arg);
                send_reply(&mut stream, 150, "Abriendo conexión de datos para STOR")?;
                let (mut data_stream, _) = listener.accept()?;
            ...
            "NOOP" => {
                send_reply(&mut stream, 200, "OK")?;
            }
            "QUIT" => {
                send_reply(&mut stream, 221, "Adiós")?;
                break;
            }
            _ => {
                send_reply(&mut stream, 502, "Comando no implementado")?;
            }
        }
    }
    Ok(())
...
fn main() -> io::Result<()> {
    ...
    println!("Raíz del FTP: {}", root_dir.display());
            ...
            Ok(stream) => {
                ...
                thread::spawn(move || {
                    if let Err(e) = handle_client(stream, root_clone) {
                        eprintln!("Error en sesión FTP: {e}");
                    }
                });
            }
            Err(e) => {
