...
describe('login', () => {
  it('devuelve { user, token } desenvuelto', async () => {
    ...
    const resultado = await login('juan', 'secreto123');
  ...
  it('manda usuario y contraseña como JSON al endpoint correcto', async () => {
    ...
    await login('juan', 'secreto123');

    const [url, options] = global.fetch.mock.calls[0];
    expect(url).toBe('/api/auth/login');
    expect(options.method).toBe('POST');
    expect(JSON.parse(options.body)).toEqual({ username: 'juan', password: 'secreto123' });
  });
...
describe('register', () => {
  it('devuelve el usuario creado con su rol', async () => {
    ...
    const resultado = await register('nueva', 'secreto123');
  ...
  it('llama al endpoint de registro, no al de login', async () => {
    ...
    await register('nueva', 'secreto123');
  ...
  it('propaga el 409 de usuario ya existente', async () => {
    ...
    await expect(register('juan', 'secreto123')).rejects.toThrow('El usuario ya existe');
  });

  it('usa un mensaje por defecto si el servidor no manda ninguno', async () => {
    global.fetch = mockFetch(500, { success: false });
    await expect(register('x', 'secreto123')).rejects.toThrow('Error al registrarse');
  });
});
