...
# Aqui es un requisito, no un descuido: la aplicacion recibe el audio del otro
# participante desde otro equipo de la red local. Enlazar a 127.0.0.1 la
# dejaria sin funcion alguna.
#
# El riesgo asumido no es el enlace, sino que no hay autenticacion: cualquiera
# dentro de la misma red local puede unirse o inyectar audio. Es aceptable en
# una red domestica, que es el escenario para el que esta pensado. NO lo uses
# en una red que no controles.
...
def start_TCP_server(_host_ip, port):
    """
    Levanta el servidor TCP para gestionar conexiones del cliente y reenviar mensajes de control.
    """
    global current_client, current_client_ip
        ...
        while not stop_event.is_set():
            try:
                ...
                if current_client is not None:
                    logging.warning("Ya hay un cliente conectado. Rechazando conexión de %s", ip)
                    client_socket.close()
                    continue
                
                logging.info("Cliente conectado desde: %s", ip)
...
def handle_client(client_socket, ip):
    """
    Maneja la comunicación con el cliente conectado en el modelo 1vs1.
    """
    global current_client, current_client_ip, host_info, is_host

    try:
        logging.info(f"[TCP] Manejando cliente {ip}")

        # Si no hay host aún, este cliente podría convertirse en host
        with lock:
            if not host_info:
                timestamp = time.time()
                update_host_info(ip, timestamp, True)  # Cambiar a True si este cliente debe ser host
        ...
        while not stop_event.is_set():
            try:
                ...
                logging.debug(f"[TCP] Mensaje de {ip}: {message}")

                # Aquí puedes incluir procesamiento de comandos/mensajes si lo deseas
                # process_tcp_message(message, ip)

            except ConnectionResetError:
                logging.warning(f"[TCP] Conexión reiniciada por {ip}")
                break
            except Exception as e:
                logging.error(f"[TCP] Error al recibir datos de {ip}: {e}")
                break
        ...
        logging.info(f"[TCP] Conexión con {ip} finalizada.")
...
class App(tk.Tk):
    ...
    def check_role_change(self):
        ...
        if role_changed.is_set():
            if current_role:
                self.status_label.config(text=f"Eres el {current_role}")
            role_changed.clear()
