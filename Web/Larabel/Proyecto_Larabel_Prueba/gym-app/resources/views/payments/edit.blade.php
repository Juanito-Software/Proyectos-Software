...
<div class="py-12">
    <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg p-6">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">Editar Pago</h1>
            ...
            <form action="{{ route('admin.payments.update', $payment->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="inscription_id" required
                        ...
                        @foreach($inscriptions as $ins)
                            <option value="{{ $ins->id }}" {{ old('inscription_id', $payment->inscription_id) == $ins->id ? 'selected' : '' }}>
                                {{ $ins->client->name ?? '—' }} — {{ $ins->classe->name ?? '—' }} ({{ optional($ins->date)->format('Y-m-d') ?? 'sin fecha' }})
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="amount" step="0.01" value="{{ old('amount', $payment->amount) }}" required
                           class="w-full mt-1 rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300" />
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Fecha del Pago *</label>
                    <input type="date" name="date" value="{{ old('date', optional($payment->date)->format('Y-m-d')) }}" required
                           class="w-full mt-1 rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300" />
                </div>
                ...
                <div class="flex justify-between pt-4">
                    ...
                    <button type="submit"
                            class="bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-lg">Actualizar</button>
                </div>
            </form>
        </div>
    </div>
